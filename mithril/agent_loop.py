...
DEPTH_CRITIC_PROMPT = (
    ...
    "10. REMEDIAL SPECIFICITY: for every recommendation or required action, did you name the SPECIFIC action, standard, "
    "version, or approval type the matter actually requires (e.g. 'obtain WRITTEN regulator approval', 'mandate "
    "TLS 1.3') rather than a generic one ('notify', 'upgrade', 'add controls')?\n"
    "11. CROSS-DOCUMENT PROVENANCE: for every figure, threshold, defined term, or right, did you trace it to its "
    "ORIGIN across ALL documents (not just a pairwise comparison) — e.g. note when a value was carried over from a "
    "prior/related instrument, not only that two current documents differ?\n"
    "12. QUANTITATIVE PRECISION: where a specific figure is stated or derivable, did you give the single exact "
    "computed value (recompute with python3) rather than a range or approximation?\n\n"
